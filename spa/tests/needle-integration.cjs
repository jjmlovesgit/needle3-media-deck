...
(async()=>{
 ...
 try{
  ...
  const titles=await page.locator('.library-item .library-title').allTextContents();
  const uniqueTitle=titles.filter(title=>titles.filter(value=>value===title).length===1).sort((a,b)=>a.length-b.length)[0];
  assert(uniqueTitle,'the integration library needs one unique title');
  const samples=[];
  ...
  await run('Hide the library');assert.equal(await page.locator('#libraryPanel').evaluate(el=>!el.hidden&&!el.open),true);
  await run('Show the library');assert.equal(await page.locator('#libraryPanel').evaluate(el=>!el.hidden&&el.open),true);
  await run('Show karaoke files');assert.equal(await page.locator('[data-filter="karaoke_mp4"]').getAttribute('aria-pressed'),'true');
  ...
  await page.waitForFunction(()=>!document.querySelector('#commandRun').disabled,null,{timeout:60000});
  assert.equal(await page.locator('#media').evaluate(el=>el.paused),true,'Needle-routed pause should stop playback');
  ...
  assert.match(await page.locator('#commandResult').textContent(),/Volume 42%/);
  assert.equal(await page.locator('#media').evaluate(el=>el.volume),.42);
