...
test('catalog metadata replaces staged filename inference without exposing source paths', async () => {
  ...
  try {
    ...
    await writeFile(path.join(root, 'catalog.json'), JSON.stringify({ items: [{ id: 'catalog-item-id', relativePath,
    ...
    assert.deepEqual({ catalogId: result.items[0].catalogId, title: result.items[0].title, artist: result.items[0].artist, kind: result.items[0].kind, metadataSource: result.items[0].metadataSource,
      durationMs: result.items[0].durationMs, musicBrainzRecordingId: result.items[0].musicBrainzRecordingId },
      { catalogId: 'catalog-item-id', title: 'Example Track', artist: 'Example Artist', kind: 'karaoke_mp4', metadataSource: 'Catalog', durationMs: 123000,
