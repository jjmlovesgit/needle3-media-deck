...
class KaraokeCoreTests(unittest.TestCase):
    ...
    def test_local_playback_page_supports_audio_downloads(self):
        ...
        self.assertIn("/api/wakeword/wait", page)
        self.assertIn("stt-prewarmed", page)
        self.assertIn("setTimeout(finalizePushToTalk,200)", page)
        self.assertIn("setTimeout(()=>{if(!voiceRecognition)return;try{speechLog('recognition-stop-called'", page)
