...
class KaraokeCoreTests(unittest.TestCase):
    ...
    def test_local_playback_page_wraps_video_in_console(self):
        ...
        self.assertIn("eq-signal-", page)
        self.assertIn("eq-signal-line", page)
        self.assertIn("stroke-linecap:butt", page)
        self.assertNotIn("stroke-dasharray:3 2", page)
        self.assertIn("function updateEqSignalLines()", page)
        self.assertIn("function eqBandEnergy(data,band)", page)
        self.assertIn("Math.pow(raw,1.7)", page)
        self.assertIn("function eqChannelPulseFloor(data)", page)
        self.assertIn("channelFloors=channels.map(eqChannelPulseFloor)", page)
        self.assertIn("Math.max(eqBandEnergy(channels[channel],band),channelFloors[channel])", page)
        self.assertIn("floorY=eqY(-12)", page)
        self.assertIn("settingY=eqY(eqBypassed?-12:eqGains[channel][band]),y=floorY+(settingY-floorY)*level", page)
        self.assertIn("line.setAttribute('y1',floorY.toFixed(1))", page)
        self.assertIn("leftAnalyser.fftSize=rightAnalyser.fftSize=2048", page)
    ...
    def test_local_playback_page_supports_audio_downloads(self):
        ...
        self.assertIn("voiceDuckedVolume", page)
        self.assertIn("media.volume=Math.min(media.volume,.05)", page)
        self.assertIn("playback-ducked", page)
        self.assertIn("playback-volume-restored", page)
        self.assertNotIn("playback-isolated", page)
        ...
        self.assertIn("max-height:min(420px,60vh)", page)
        self.assertIn("scrollbar-color:#087da1", page)
        self.assertIn(".command-panel summary .command-tooltip", page)
        self.assertIn("left:0;right:0;display:block;box-sizing:border-box;width:auto", page)
